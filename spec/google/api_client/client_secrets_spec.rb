...
require 'google2/api_client/client_secrets'

FIXTURES_PATH = File.expand_path('../../../fixtures', __FILE__)

RSpec.describe Google2::APIClient::ClientSecrets do
  describe '::new' do
    ...
    context 'without options' do
      subject { Google2::APIClient::ClientSecrets.new(data) }
    ...
    context 'with options' do
      context 'option keys are string' do
        ...
        subject { Google2::APIClient::ClientSecrets.new(string_options) }
      ...
      context 'option keys are symbol' do
        ...
        subject { Google2::APIClient::ClientSecrets.new(symbol_options) }
  ...
  context 'with JSON file' do
    ...
    subject(:secrets) { Google2::APIClient::ClientSecrets.load(file)}
  ...
  context 'with no existing JSON file' do
    it 'should raise exception' do
      ...
      expect { Google2::APIClient::ClientSecrets.load(file) }.to raise_exception(Errno::ENOENT)
    end
  end
  ...
  context 'with invalid JSON file' do
    it 'should raise exception' do
      ...
      expect { Google2::APIClient::ClientSecrets.load(file) }.to raise_exception(JSON::ParserError)
    end
  end
  ...
  context 'with folder name, which have json file in parents' do
    it 'should load the correct client id' do
      ...
      secrets = Google2::APIClient::ClientSecrets.load(folder)
      expect(secrets.client_id).to be == '898243283568.apps.googleusercontent.com'
    end
  end
  ...
  context 'with folder wihout client_secrets.json' do
    it "should raise exception", fakefs: true do
      FileUtils.mkdir("/tmp")
      expect { Google2::APIClient::ClientSecrets.load('/tmp') }.to raise_exception(ArgumentError)
    end
  end

...
require 'spec_helper'
require 'google2/apis/core/upload'
...
RSpec.describe Google2::Apis::Core::UploadIO do
  context 'from_file' do
    let(:upload_io) { Google2::Apis::Core::UploadIO.from_file(file) }
    ...
    context 'with text file' do
      ...
      it 'should allow overriding the mime type' do
        io = Google2::Apis::Core::UploadIO.from_file(file, content_type: 'application/json')
    ...
    context 'with unknown type' do
      ...
      it 'should setup length of the stream' do
        upload_io = Google2::Apis::Core::UploadIO.from_file(file)
  ...
  context 'from_io' do
    ...
    context 'with i/o stream' do
      ...
      it 'should setup default content-type' do
        upload_io = Google2::Apis::Core::UploadIO.from_io(io)
        expect(upload_io.content_type).to eql Google2::Apis::Core::UploadIO::OCTET_STREAM_CONTENT_TYPE
      end

      it 'should allow overring the mime type' do
        upload_io = Google2::Apis::Core::UploadIO.from_io(io, content_type: 'application/x-gzip')
...
RSpec.describe Google2::Apis::Core::RawUploadCommand do
  include TestHelpers
  include_context 'HTTP client'

  let(:command) do
    command = Google2::Apis::Core::RawUploadCommand.new(:post, 'https://www.googleapis.com/zoo/animals')
  ...
  context('with invalid input') do
    ...
    it 'should raise client error' do
      expect { command.execute(client) }.to raise_error(Google2::Apis::ClientError)
    end
  end
end

RSpec.describe Google2::Apis::Core::MultipartUploadCommand do
  include TestHelpers
  include_context 'HTTP client'

  let(:command) do
    command = Google2::Apis::Core::MultipartUploadCommand.new(:post, 'https://www.googleapis.com/zoo/animals')
...
RSpec.describe Google2::Apis::Core::ResumableUploadCommand do
  include TestHelpers
  include_context 'HTTP client'

  let(:command) do
    command = Google2::Apis::Core::ResumableUploadCommand.new(:post, 'https://www.googleapis.com/zoo/animals')
    command.upload_source = StringIO.new('Hello world')
    command.upload_content_type = 'text/plain'
    command
  end
  ...
  context 'with non-retriable authorization error on start' do
    ...
    it 'should propagate the original error' do
      expect { command.execute(client) }.to raise_error Google2::Apis::AuthorizationError
    end
  end

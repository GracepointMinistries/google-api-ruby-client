...
RSpec.describe Google2::Apis::Core::BaseService do
  include TestHelpers

  let(:service) { Google2::Apis::Core::BaseService.new('https://www.googleapis.com/', '') }

  before do
    Google2::Apis::ClientOptions.default.application_name = 'test'
    Google2::Apis::ClientOptions.default.application_version = '1.0'
  end
  ...
  it 'should inherit authorization' do
    Google2::Apis::RequestOptions.default.authorization = 'a token'
  ...
  context 'when making simple commands' do
    ...
    it 'should return the correct command type' do
      expect(command).to be_an_instance_of(Google2::Apis::Core::ApiCommand)
    end
  ...
  context 'when making download commands' do
    ...
    it 'should return the correct command type' do
      expect(command).to be_an_instance_of(Google2::Apis::Core::DownloadCommand)
    end
  ...
  context 'when making upload commands' do
    ...
    it 'should return the correct command type' do
      expect(command).to be_an_instance_of(Google2::Apis::Core::ResumableUploadCommand)
    end
...
EOF
    ...
    it 'should disallow uploads in batch' do
      ...
      end.to raise_error(Google2::Apis::ClientError)
    end
    ...
    it 'should use multipart upload' do
      expect do |b|
        service.batch_upload do |service|
          ...
          command.upload_source = StringIO.new('test')
          command.upload_content_type = 'text/plain'
          expect(command).to be_an_instance_of(Google2::Apis::Core::MultipartUploadCommand)
          service.send(:execute_or_queue_command, command, &b)
        end
      end.to yield_with_args('Hello', nil)
    end

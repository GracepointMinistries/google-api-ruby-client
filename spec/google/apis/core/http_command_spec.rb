...
require 'spec_helper'
require 'google2/apis/core/http_command'
require 'hurley/test'

RSpec.describe Google2::Apis::Core::HttpCommand do
  include TestHelpers
  include_context 'HTTP client'

  context('with credentials') do
    let(:command) do
      command = Google2::Apis::Core::HttpCommand.new(:get, 'https://www.googleapis.com/zoo/animals')
  ...
  context('with a successful response') do
    let(:command) do
      Google2::Apis::Core::HttpCommand.new(:get, 'https://www.googleapis.com/zoo/animals')
    end
  ...
  context('with server errors') do
    ...
    it 'should raise error if retries exceeded' do
      ...
      expect { command.execute(client) }.to raise_error(Google2::Apis::ServerError)
    end
  ...
  context('with too many redirects') do
    ...
    it 'should raise error if retries exceeded' do
      expect { command.execute(client) }.to raise_error(Google2::Apis::RedirectError)
    end
  end
  ...
  context('with no server response') do
    ...
    it 'should raise transmission error' do
      expect { command.execute(client) }.to raise_error(Google2::Apis::TransmissionError)
    end
  end
  ...
  context('with client errors') do
    ...
    it 'should call block if present' do
      expect { |b| command.execute(client, &b) }.to yield_with_args(nil, an_instance_of(Google2::Apis::ClientError))
    end
  ...
  it 'should not remove initial query parameters' do
    stub_request(:get, 'https://www.googleapis.com/zoo/animals?a=1&a=2&a=3&foo=bar')
      .to_return(status: [200, ''])
    command = Google2::Apis::Core::HttpCommand.new(:get, 'https://www.googleapis.com/zoo/animals?foo=bar')
    command.query['a'] = [1,2,3]
    command.execute(client)
  end
  ...
  it 'should form encode parameters when method is POST and no body present' do
    stub_request(:post, 'https://www.googleapis.com/zoo/animals')
        ...
        .to_return(status: [200, ''])
    command = Google2::Apis::Core::HttpCommand.new(:post, 'https://www.googleapis.com/zoo/animals')
    command.query['a'] = [1,2,3]

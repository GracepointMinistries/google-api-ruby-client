...
RSpec.describe Google2::Apis::Core::HttpCommand do
  ...
  context('with a request body') do
    let(:command) do
      ...
      request.value = 'hello'
      command = Google2::Apis::Core::ApiCommand.new(:post, 'https://www.googleapis.com/zoo/animals')
  ...
  context('with a JSON response') do
    let(:command) do
      command = Google2::Apis::Core::ApiCommand.new(:get, 'https://www.googleapis.com/zoo/animals')
  ...
  context('with a rate limit response') do
    let(:command) do
      Google2::Apis::Core::ApiCommand.new(:get, 'https://www.googleapis.com/zoo/animals')
    end

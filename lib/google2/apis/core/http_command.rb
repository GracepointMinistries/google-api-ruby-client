  ...
  module Apis
    module Core
      ...
      class HttpCommand
        ...
        RETRIABLE_ERRORS = [Google2::Apis::ServerError, Google2::Apis::RateLimitError, Google2::Apis::TransmissionError]
        ...
        def initialize(method, url, body: nil)
          self.options = Google2::Apis::RequestOptions.default.dup
          self.url = url
          ...
          begin
            Retriable.retriable tries: options.retries + 1,
              ...
              Retriable.retriable tries: auth_tries,
                                  on: [Google2::Apis::AuthorizationError],
        ...
        def check_status(status, header = nil, body = nil, message = nil)
          ...
          when 301, 302, 303, 307
            ...
            raise Google2::Apis::RedirectError.new(message, status_code: status, header: header, body: body)
          when 401
            message ||= 'Unauthorized'
            raise Google2::Apis::AuthorizationError.new(message, status_code: status, header: header, body: body)
          when 304, 400, 402...500
            message ||= 'Invalid request'
            raise Google2::Apis::ClientError.new(message, status_code: status, header: header, body: body)
          when 500...600
            message ||= 'Server error'
            raise Google2::Apis::ServerError.new(message, status_code: status, header: header, body: body)
          else
            logger.warn(sprintf('Encountered unexpected status code %s', status))
            message ||= 'Unknown error'
            raise Google2::Apis::TransmissionError.new(message, status_code: status, header: header, body: body)
          end
        end
        ...
        def error(err, rethrow: false, &block)
          ...
          err = Google2::Apis::TransmissionError.new(err) if err.is_a?(Hurley::ClientError) || err.is_a?(SocketError)

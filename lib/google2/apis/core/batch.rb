  ...
  module Apis
    module Core
      ...
      class BatchCommand < HttpCommand
        ...
        #
        # @param [Google2::Apis::Core::HttpCommand] call API Request to add
        # @yield [result, err] Result & error when response available
        # @return [Google2::Apis::Core::BatchCommand] self
        def add(call, &block)
        ...
        # @return [void]
        # @raise [Google2::Apis::BatchError] if batch is empty
        def prepare!
        ...
        def ensure_valid_command(command)
          if command.is_a?(Google2::Apis::Core::BaseUploadCommand) || command.is_a?(Google2::Apis::Core::DownloadCommand)
            fail Google2::Apis::ClientError, 'Can not include media requests in batch'
          end
          fail Google2::Apis::ClientError, 'Invalid command object' unless command.is_a?(HttpCommand)
        end
      ...
      class BatchUploadCommand < BatchCommand
        def ensure_valid_command(command)
          fail Google2::Apis::ClientError, 'Can only include upload commands in batch' \
            unless command.is_a?(Google2::Apis::Core::BaseUploadCommand)
        end
      ...
      class CallSerializer
        ...
        #
        # @param [Google2::Apis::Core::HttpCommand] call
